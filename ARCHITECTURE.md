...
## Playwright 与 Midscene 协作约束

- `framework/midscene/` 只提供跨平台的 Agent 适配和配置判断，不包含任何平台业务定位或流程。
- Playwright 是确定性主路径：正式 P0/Smoke 用例的页面操作、等待、接口响应和最终断言必须由 Playwright/Page Object 完成。
- Midscene 是可选辅助层，只用于探索、语义观察、视觉兜底和未稳定建模页面的临时操作；不得替代 `locators/`、`pages/` 的长期资产沉淀。
- 业务代码不得直接导入 `@midscene/web`；统一使用认证 Fixture 注入的 `midscene` 代理。代理未配置时为 `undefined`，用例应通过 `test.skip` 或显式分支跳过 AI 专项，不影响普通回归。
- 启用 AI 操作后必须紧跟 Playwright 的确定性断言。AI 结果不得单独作为 P0 放行依据，也不得以增加重试次数处理模型不稳定。
- Midscene 模型 Key、模型地址、运行目录和缓存均使用环境变量或 CI Secret；禁止写入源码、README、报告和版本库。
